...
pub fn register(js_ctx: &JsContext) -> Result<()> {
    js_ctx.with(|ctx| -> Result<()> {
        ...
        g.set("__cm_fs_read_text", Function::new(ctx.clone(), |ctx: Ctx<'_>, path: String| -> rquickjs::Result<String> {
            fs::read_to_string(&path).map_err(|e| throw(&ctx, e))
        })?)?;

        g.set("__cm_fs_write_text", Function::new(ctx.clone(), |ctx: Ctx<'_>, path: String, content: String| -> rquickjs::Result<()> {
        ...
        g.set("__cm_fs_exists", Function::new(ctx.clone(), |path: String| -> bool {
            Path::new(&path).exists()
        })?)?;

        g.set("__cm_fs_is_file", Function::new(ctx.clone(), |path: String| -> bool {
            Path::new(&path).is_file()
        })?)?;

        g.set("__cm_fs_is_dir", Function::new(ctx.clone(), |path: String| -> bool {
            Path::new(&path).is_dir()
        })?)?;

        g.set("__cm_fs_read_dir", Function::new(ctx.clone(), |ctx: Ctx<'_>, path: String| -> rquickjs::Result<Vec<String>> {
            let mut out = Vec::new();
        ...
        g.set("__cm_fs_mkdir", Function::new(ctx.clone(), |ctx: Ctx<'_>, path: String, recursive: bool| -> rquickjs::Result<()> {
            if recursive {
        ...
        g.set("__cm_fs_rm", Function::new(ctx.clone(), |ctx: Ctx<'_>, path: String, recursive: bool| -> rquickjs::Result<()> {
            let p = Path::new(&path);
            if p.is_dir() {
                if recursive {
        ...
        g.set("__cm_fs_rename", Function::new(ctx.clone(), |ctx: Ctx<'_>, from: String, to: String| -> rquickjs::Result<()> {
        ...
        g.set("__cm_fs_stat", Function::new(ctx.clone(), |ctx: Ctx<'_>, path: String| -> rquickjs::Result<String> {
        ...
        g.set("__cm_fs_home_dir", Function::new(ctx.clone(), || -> Option<String> {
            dirs::home_dir().and_then(|p| p.to_str().map(|s| s.to_string()))
        })?)?;

        g.set("__cm_fs_app_data_dir", Function::new(ctx.clone(), || -> Option<String> {
            dirs::data_dir().and_then(|p| p.to_str().map(|s| s.to_string()))
        })?)?;

        g.set("__cm_fs_app_config_dir", Function::new(ctx.clone(), || -> Option<String> {
            dirs::config_dir().and_then(|p| p.to_str().map(|s| s.to_string()))
        })?)?;

        g.set("__cm_fs_app_cache_dir", Function::new(ctx.clone(), || -> Option<String> {
            dirs::cache_dir().and_then(|p| p.to_str().map(|s| s.to_string()))
        })?)?;

        g.set("__cm_fs_temp_dir", Function::new(ctx.clone(), || -> String {
