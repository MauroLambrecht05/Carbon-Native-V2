...
pub fn register(js_ctx: &JsContext) -> Result<()> {
    js_ctx.with(|ctx| -> Result<()> {
        ...
        g.set("__cm_proc_exec", Function::new(ctx.clone(), |ctx: Ctx<'_>, cmd: String, args_json: String, cwd: String| -> rquickjs::Result<String> {
            ...
            let stdout = serde_json::to_string(&String::from_utf8_lossy(&out.stdout).into_owned()).unwrap_or_default();
            let stderr = serde_json::to_string(&String::from_utf8_lossy(&out.stderr).into_owned()).unwrap_or_default();
            let code = out.status.code().unwrap_or(-1);
            Ok(format!(r#"{{"stdout":{},"stderr":{},"code":{}}}"#, stdout, stderr, code))
        })?)?;

        // spawn: returns u32 handle. The handle is the only way to talk
        // to the child from now on.
        g.set("__cm_proc_spawn", Function::new(ctx.clone(), |ctx: Ctx<'_>, cmd: String, args_json: String, cwd: String| -> rquickjs::Result<u32> {
            let args = parse_args(&args_json);
            let mut c = Command::new(&cmd);
            c.args(&args);
            if !cwd.is_empty() {
        ...
        g.set("__cm_proc_write_stdin", Function::new(ctx.clone(), |ctx: Ctx<'_>, id: u32, data: String| -> rquickjs::Result<u32> {
        ...
        g.set("__cm_proc_read_stdout", Function::new(ctx.clone(), |id: u32| -> String {
        ...
        g.set("__cm_proc_read_stderr", Function::new(ctx.clone(), |id: u32| -> String {
            let reg = registry().lock().unwrap_or_else(|e| e.into_inner());
            if let Some(h) = reg.get(&id) {
        ...
        g.set("__cm_proc_kill", Function::new(ctx.clone(), |id: u32| -> rquickjs::Result<()> {
            let mut reg = registry().lock().unwrap_or_else(|e| e.into_inner());
        ...
        g.set("__cm_proc_wait", Function::new(ctx.clone(), |ctx: Ctx<'_>, id: u32| -> rquickjs::Result<i32> {
            let mut h = match registry().lock().unwrap_or_else(|e| e.into_inner()).remove(&id) {
                Some(h) => h,
                None => return Ok(-1),
            };
        ...
        g.set("__cm_proc_try_status", Function::new(ctx.clone(), |id: u32| -> i32 {
            let mut reg = registry().lock().unwrap_or_else(|e| e.into_inner());
            if let Some(h) = reg.get_mut(&id) {
        ...
        g.set("__cm_proc_pid_self", Function::new(ctx.clone(), || -> u32 {
            std::process::id()
        })?)?;
        ...
        g.set("__cm_proc_relaunch_self", Function::new(ctx.clone(), |ctx: Ctx<'_>| -> rquickjs::Result<()> {
