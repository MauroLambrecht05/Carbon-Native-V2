...
pub fn register(js_ctx: &JsContext) -> Result<()> {
    js_ctx.with(|ctx| -> Result<()> {
        ...
        g.set("__cm_keychain_set", Function::new(ctx.clone(), |ctx: Ctx<'_>, service: String, account: String, password: String| -> rquickjs::Result<()> {
        ...
        g.set("__cm_keychain_get", Function::new(ctx.clone(), |ctx: Ctx<'_>, service: String, account: String| -> rquickjs::Result<Option<String>> {
            let entry = Entry::new(&service, &account).map_err(|e| throw(&ctx, e))?;
        ...
        // an error.
        g.set("__cm_keychain_delete", Function::new(ctx.clone(), |ctx: Ctx<'_>, service: String, account: String| -> rquickjs::Result<()> {
            let entry = Entry::new(&service, &account).map_err(|e| throw(&ctx, e))?;
