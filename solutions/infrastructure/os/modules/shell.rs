...
pub fn register(js_ctx: &JsContext) -> Result<()> {
    js_ctx.with(|ctx| -> Result<()> {
        ...
        g.set("__cm_shell_open", Function::new(ctx.clone(), |ctx: Ctx<'_>, target: String| -> rquickjs::Result<()> {
            opener::open(&target).map_err(|e| throw(&ctx, e))
        })?)?;

        g.set("__cm_shell_reveal", Function::new(ctx.clone(), |ctx: Ctx<'_>, path: String| -> rquickjs::Result<()> {
            let p = Path::new(&path);
        ...
        g.set("__cm_shell_resolve", Function::new(ctx.clone(), |path: String| -> Option<String> {
            std::fs::canonicalize(&path).ok().and_then(|p| p.to_str().map(|s| s.to_string()))
        })?)?;
