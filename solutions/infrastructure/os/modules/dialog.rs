...
pub fn register(js_ctx: &JsContext) -> Result<()> {
    js_ctx.with(|ctx| -> Result<()> {
        ...
        g.set("__cm_dialog_open_file", Function::new(ctx.clone(), |opts_json: String| -> Option<String> {
            let opts = parse_opts(&opts_json);
            build_dialog(&opts).pick_file().and_then(|p| p.to_str().map(|s| s.to_string()))
        })?)?;

        g.set("__cm_dialog_open_files", Function::new(ctx.clone(), |opts_json: String| -> Vec<String> {
            let opts = parse_opts(&opts_json);
            build_dialog(&opts).pick_files()
                .map(|v| v.into_iter().filter_map(|p| p.to_str().map(|s| s.to_string())).collect())
                .unwrap_or_default()
        })?)?;

        g.set("__cm_dialog_open_dir", Function::new(ctx.clone(), |opts_json: String| -> Option<String> {
            let opts = parse_opts(&opts_json);
            build_dialog(&opts).pick_folder().and_then(|p| p.to_str().map(|s| s.to_string()))
        })?)?;

        g.set("__cm_dialog_save_file", Function::new(ctx.clone(), |opts_json: String| -> Option<String> {
            let opts = parse_opts(&opts_json);
            build_dialog(&opts).save_file().and_then(|p| p.to_str().map(|s| s.to_string()))
        })?)?;

        // Message box with a single OK button. `level` is "info" /
        // "warning" / "error" — anything else maps to info.
        g.set("__cm_dialog_message", Function::new(ctx.clone(), |title: String, body: String, level: String| -> () {
        ...
        g.set("__cm_dialog_confirm", Function::new(ctx.clone(), |title: String, body: String| -> bool {
