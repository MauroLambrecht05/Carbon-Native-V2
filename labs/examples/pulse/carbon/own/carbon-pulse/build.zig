...
pub fn build(b: *std.Build) void {
    ...
    const lib = b.addLibrary(.{
        .linkage = .dynamic,
        // carbon_pulse, not carbon-pulse: this is a hyphenated project name inside
        // build.zig, which SdkTemplateSource.ts's render() treats as a
        // non-source file (isSourceFile only matches src/) and so gives the
        // slug form to plain carbon-pulse — but the shared-library FILENAME this
        ...
        // is exactly what carbon_pulse always resolves to regardless of file.
