...
// `carbon plugin install` / `carbon plugin add deep-link` copies into the
...
pub fn build(b: *std.Build) void {
    ...
    const lib = b.addLibrary(.{
        .linkage = .dynamic,
        // Must match PluginName.from("deep-link").crate exactly — hyphens
        // become underscores (see PluginName.ts: "Rust/Zig identifier
        // form — hyphens are not legal in either"; see global-shortcuts'
        // build.zig for the same fix). This is the shared-library
        // filename InstallPluginUseCase's locateArtifact() searches for.
        // The plugin's own carbon-plugin.toml `name` field ("deep-link",
        // hyphenated) is what that name comes from.
        .name = "deep_link",
        .root_module = lib_mod,
    });
