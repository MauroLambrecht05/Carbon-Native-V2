...
// `carbon plugin install` / `carbon plugin add global-shortcuts` copies into the
...
pub fn build(b: *std.Build) void {
    ...
    const lib = b.addLibrary(.{
        .linkage = .dynamic,
        // Must match PluginName.from("global-shortcuts").crate exactly —
        // hyphens become underscores (see PluginName.ts: "Rust/Zig
        // identifier form — hyphens are not legal in either") — this is
        // the shared-library filename InstallPluginUseCase's
        // locateArtifact() searches for by that derived name. The
        // plugin's own carbon-plugin.toml `name` field ("global-
        // shortcuts", hyphenated) is what that name comes from.
