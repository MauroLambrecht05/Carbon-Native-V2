// global-shortcuts — system-wide keyboard shortcuts that fire even when
// the app is unfocused or minimized.
//
//   carbon plugin add global-shortcuts
//
...
// already prototyped in labs/examples/pulse/plugins/carbon-hotkey (a
...
//
//   zig build                            build it
//   carbon plugin add global-shortcuts   fetch + build + install
...
pub const CFG = sdk.manifest.Config{
    ...
    // the deep-link plugin's own "register" export when both are loaded
...
// labs/examples/pulse/plugins/carbon-hotkey prototyped, since this plugin
...
test "the manifest declares both points and no stray capability" {
    ...
    try std.testing.expect(std.mem.indexOf(u8, json, "\"name\":\"global-shortcuts\"") != null);
