...
// `carbon plugin install` / `carbon plugin add file-search` copies into the
...
pub fn build(b: *std.Build) void {
    ...
    const lib = b.addLibrary(.{
        .linkage = .dynamic,
        // Must match PluginName.from("file-search").crate exactly — this is
        // the shared-library filename InstallPluginUseCase's
        // locateArtifact() searches for by that derived name. The plugin's
        // own carbon-plugin.toml `name` field ("file-search") is what that
        // name comes from.
        .name = "file_search",
        .root_module = lib_mod,
    });
    ...
    // regex/glob/gitignore unit tests those modules carry.
    ...
    const test_step = b.step("test", "Check the plugin against the extension-point registry, and run the regex/glob/gitignore unit tests");
