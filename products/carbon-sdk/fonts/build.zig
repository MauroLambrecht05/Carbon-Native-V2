...
// `carbon plugin install` / `carbon plugin add fonts` copies into the app
// and the runtime dlopens.
...
pub fn build(b: *std.Build) void {
    ...
    const lib = b.addLibrary(.{
        .linkage = .dynamic,
        // Must match PluginName.from("fonts").crate exactly — this is the
        // shared-library filename InstallPluginUseCase's locateArtifact()
        // searches for by that derived name. The plugin's own carbon-
        // plugin.toml `name` field ("fonts") is what that name comes from.
        .name = "fonts",
        .root_module = lib_mod,
    });
