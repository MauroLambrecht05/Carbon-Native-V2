...
//
//   carbon plugin add notification
//
...
//
//   zig build                       build it
//   carbon plugin add notification  fetch + build + install into the app
...
test "the manifest declares both points and no stray capability" {
    ...
    try std.testing.expect(std.mem.indexOf(u8, json, "\"name\":\"notification\"") != null);
