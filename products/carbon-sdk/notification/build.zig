...
// `carbon plugin install` / `carbon plugin add notification` copies into the
...
pub fn build(b: *std.Build) void {
    ...
    const lib = b.addLibrary(.{
        .linkage = .dynamic,
        // Must match PluginName.from("notification").crate exactly — this is
        // the shared-library filename InstallPluginUseCase's
        // locateArtifact() searches for by that derived name. The plugin's
        // own carbon-plugin.toml `name` field ("notification") is what that
        // name comes from.
        .name = "notification",
        .root_module = lib_mod,
    });
