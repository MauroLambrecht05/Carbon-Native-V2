...
// `carbon plugin install` / `carbon plugin add biometrics` copies into the
...
pub fn build(b: *std.Build) void {
    ...
    const lib = b.addLibrary(.{
        .linkage = .dynamic,
        // Must match PluginName.from("biometrics").crate exactly — this is
        // the shared-library filename InstallPluginUseCase's
        // locateArtifact() searches for by that derived name. The plugin's
        // own carbon-plugin.toml `name` field ("biometrics") is what that
        // name comes from.
        .name = "biometrics",
        .root_module = lib_mod,
    });
