// search — in-app gitignore-aware grep/glob/search, as an opt-in plugin
// instead of an always-on runtime capability. Plugin name "search";
// installs the "carbon:file-search" JS module (unchanged from before this
// plugin's own folder/install-name was shortened).
//
//   carbon plugin add search
//
...
//
//   zig build                    build it
//   carbon plugin add search     fetch + build + install into the current app
