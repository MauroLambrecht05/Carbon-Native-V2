// deeplink — custom URL scheme handling (`myapp://...`). Plugin name
// "deeplink"; installs the "carbon:deep-link" JS module (unchanged from
// before this plugin's own folder/install-name was shortened).
//
//   carbon plugin add deeplink
//
...
//
//   zig build                  build it
//   carbon plugin add deeplink fetch + build + install into the current app
...
pub const CFG = sdk.manifest.Config{
    ...
    // Prefixed, not bare "register" — collided with the shortcuts plugin's
    // own "register" export when both are loaded in the same app
...
test "the manifest declares both points and no stray capability" {
    ...
    try std.testing.expect(std.mem.indexOf(u8, json, "\"name\":\"deeplink\"") != null);
