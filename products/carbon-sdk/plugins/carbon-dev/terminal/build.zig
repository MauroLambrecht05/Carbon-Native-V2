...
// `carbon plugin install` / `carbon plugin add terminal` copies into the
...
pub fn build(b: *std.Build) void {
    ...
    const lib = b.addLibrary(.{
        .linkage = .dynamic,
        // Must match PluginName.from("terminal").crate exactly — this is
        // the shared-library filename InstallPluginUseCase's
        // locateArtifact() searches for by that derived name. The plugin's
        // own carbon-plugin.toml `name` field ("terminal") is what that
        // name comes from.
        .name = "terminal",
        .root_module = lib_mod,
    });
    ...
    // `zig build test` runs the comptime assertions in src/main.zig plus the
    // pty_win.zig unit tests (command-line quoting, environment block).
    ...
    // pty_win.zig's own tests aren't reachable through main.zig's import
    // graph (main.zig imports it but doesn't re-export its test blocks) —
    // Zig only runs `test` blocks in files reachable from the root module,
    // and imported files ARE reachable, so this actually isn't needed... but
    // kept explicit for clarity: main.zig's `const pty = @import("pty_win.zig")`
    // already pulls pty_win.zig's tests into the same test binary.
    const test_step = b.step("test", "Check the plugin against the extension-point registry, and run the pty_win.zig unit tests");
