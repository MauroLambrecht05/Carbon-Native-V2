...
// `carbon plugin install` / `carbon plugin add carbon-manifest` copies into the
...
pub fn build(b: *std.Build) void {
    ...
    const lib = b.addLibrary(.{
        .linkage = .dynamic,
        // Must match PluginName.from("carbon-manifest").crate exactly — this is
        // the shared-library filename InstallPluginUseCase's
        // locateArtifact() searches for by that derived name. The plugin's
        // own carbon-plugin.toml `name` field ("carbon-manifest") is what that
        // name comes from.
