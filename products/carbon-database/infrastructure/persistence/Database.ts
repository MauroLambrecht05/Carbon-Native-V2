// The product's own Postgres connection. Same shape as carbon-cloud's
...
// (unlike identity, which carbon-database calls carbon-cloud's real API
// for instead of keeping a second copy — see HttpIdentityClient).
 ...
 * so re-running the set is a no-op — same posture as carbon-cloud's own
 * migrate() until there's a real migration-tracking table to justify one.
 */
